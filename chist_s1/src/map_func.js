function bind_map_func(div, mp4div) {
    div
        ...
        .on("click", ".action img", function (e) { })
        .on("click", ".action", function (e) {
            ...
            if (target.hasClass("movie")) {
                ...
                var v = $("<video />").attr("width", 800).attr("height", 450).attr("controls", "controls").html(
                    '<source src="mp4/' + path + '" type="video/mp4">'
                );
                mp4div.append(v);
                ...
                var path = target.data("path");
                var title = target.data("title");
                var v = $(`<iframe src='${path}' ></iframe>`).attr("width", bodytWidth/1.8).attr("height", 650*boxscale)
        ...
        .on("mousedown", ".action_all", function (e) {
            ...
            var me = $(e.delegateTarget);
            var arr = me.data("arr_flag");
            var actions = me.find(".action").filter(function () { return !$(this).is(".movie,.googlemap,.ques"); });
            var new_arr;

...
const animate = {
  ...
  drawHousePromise3(flag, canvasId, x, y) {
    let canvasAnimHorse = document.getElementById('canvashouse3');
    let contextAnimHorse = canvasAnimHorse.getContext("2d");
  ...
  drawHorse(object, contextS, imageHorse, isStop,scale) {
    ...
    return new Promise((resolve, rej) => {
      ...
      //     contextS.scale(-1, 1);
      // }
      if(object.position.currPoint==3){
        this.btn2Play=true
      }
  ...
  imagesCanvas() {
    ...
    Object.keys(this.control).forEach(e => {
      ...
      if ((this.control[e] == true) && this.imageObj[e]) {
        ...
        if(this.canvasImagesObj[e]){          
          this.canvasObj[e].style.visibility = (this.control[e] == true)?'visible':'hidden'
        }
      }
    });
  }
